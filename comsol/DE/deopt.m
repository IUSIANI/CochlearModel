...
S_val(1)       = feval(fname,FM_pop(I_best_index,:),S_struct,Model_struct);

S_bestval = S_val(1);                 % best objective function value so far
I_nfeval  = I_nfeval + 1;
for k=2:I_NP                          % check the remaining members
  S_val(k)  = feval(fname,FM_pop(k,:),S_struct,Model_struct);
  I_nfeval  = I_nfeval + 1;
...
%-----Select which vectors are allowed to enter the new population------------
  for k=1:I_NP
      ...
      S_tempval = feval(fname,FM_ui(k,:),S_struct, Model_struct);   % check cost of competitor
      I_nfeval  = I_nfeval + 1;
...
%----Output section----------------------------------------------------------
  ...
  if (I_refresh > 0)
     if ((rem(I_iter,I_refresh) == 0) | I_iter == 1)
       fprintf(1,'\t[-] Iteration: %d,  Best: %g,  F_weight: %g,  F_CR: %g,  I_NP: %d\n',I_iter,S_bestval.FVr_oa(1),F_weight,F_CR,I_NP);
       %var(FM_pop)
       format long e;
       for n=1:I_D
          fprintf(1,'\t\t[*] best(%d) = %g\n',n,FVr_bestmem(n));
