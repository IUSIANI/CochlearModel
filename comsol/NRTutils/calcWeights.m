...
%     for k=1:length(I_k)
%         NRT_Interp_0(k) = interp1(NRT_real_k_Rec_Elect_Select,NRT_real_k_Amplitude_Select(k,:),0,InterpolationMethod);
%         for i=1:numNeurons
...
%     end
    ...
    for k=1:length(I_k)
        if isempty(find(NRT_real_k_Rec_Elect_Select==anchorElectrode))
            NRT_anchor = interp1(NRT_real_k_Rec_Elect_Select,NRT_real_k_Amplitude_Select(k,:),anchorElectrode,InterpolationMethod)*1.2; % POR QUÉ?
        else
